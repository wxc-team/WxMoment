...
(function(global) {
    ...
    global.resetMpVideoH = function(iframe) {
        var w = document.getElementById("js_content").offsetWidth,
            h = Math.ceil(w * 3 / 4);
        ...
        //iframe.style.cssText = ";position:static;";
        return false;
    }

    global.__setOriginStatus = function(data) {
        var dom_w = document.getElementById("js_content").offsetWidth,
            dom_h = Math.ceil(w * 3 / 4);
        ...
        for (var i = 0, il = iframes.length; i < il; i++) {
            ...
            if (!!src && src.indexOf("/mp/videoplayer") != -1) {
                setTimeout((function(a_tmp) {
                    return function() {
                        var w_tmp = document.getElementById("js_content").offsetWidth,
                            h_tmp = Math.ceil(w_tmp * 3 / 4);
...
// 根据播放区域高度计算出宽度，宽高比例4:3
WxVideo.prototype.setWidth = function() {
    this.playerConfig.width = this.playerConfig.height / this.sizeRate;
};

// 重置播放容器区域，只接收height参数，width由计算得出
WxVideo.prototype.resize = function(height) {
